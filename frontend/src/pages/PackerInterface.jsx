...
const TONE = {
  IDLE:      { bg: "bg-slate-800",  text: "text-slate-200", label: "READY" },
  RECORDING: { bg: "bg-red-600",    text: "text-white",     label: "RECORDING" },
  CONFIRMED: { bg: "bg-emerald-600",text: "text-white",     label: "SAVED — SCAN NEXT" },
};
  ...
  return (
    <div className="flex h-full">
      ...
      <section className="flex-1 min-w-0 flex flex-col bg-black">
        <div
          className={`flex items-center justify-between px-8 py-5 ${tone.bg} ${tone.text} transition-colors duration-200`}
        >
        ...
        <div className="px-8 py-3 bg-slate-900 border-t border-border text-xs text-slate-400 flex items-center justify-between">
          <div>
            Last scan:{" "}
      ...
      {/* ---------- RIGHT: side panel ------------------------------------- */}
      <aside className="w-[360px] shrink-0 flex flex-col border-l border-border bg-panel">
        <div className="p-5 border-b border-border">
          ...
          <select
            ...
            className="w-full rounded-md bg-slate-800 border border-border px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
          >
          ...
          {log.length === 0 ? (
            ...
            <ul className="space-y-2">
              {log.map((e, i) => (
                <li
                  key={i}
                  className="rounded-md border border-border bg-slate-900/60 px-3 py-2 text-sm"
                >
